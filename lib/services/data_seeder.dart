...
class DataSeeder {
  ...
  Future<void> seedSampleData() async {
    try {
      ...
      if (user == null) {
        ...
        print('❌ No user logged in. Please login first to seed data.');
        return;
      }

      print('✅ User authenticated: ${user.email} (UID: ${user.uid})');
      print('📱 Seeding sample listings for user: ${user.email}');
      ...
      print('📊 Existing documents found: ${existingSnapshot.docs.length}');
      
      if (existingSnapshot.docs.isNotEmpty) {
        print('⚠️ Sample data already exists for this user. Skipping seeding.');
        return;
      }

      // Add sample listings
      print('📝 Creating batch write for ${_sampleListings.length} listings...');
      ...
      for (final sampleListing in _sampleListings) {
        ...
        print('📄 Adding to batch: ${listing.name} (${docRef.id})');
        batch.set(docRef, listing.toMap());
      }

      print('💾 Committing batch to Firestore...');
      await batch.commit();
      print('✅ Successfully seeded ${_sampleListings.length} sample listings!');
  ...
  Future<void> clearAllListings() async {
    try {
      ...
      await batch.commit();
      print('Cleared ${snapshot.docs.length} listings for user: ${user.email}');
  ...
  Future<void> seedIfEmpty() async {
    try {
      ...
      if (snapshot.docs.isEmpty) {
        print('No listings found. Seeding sample data...');
        await seedSampleData();
      } else {
        print('User already has ${snapshot.docs.length} listings.');
      }
